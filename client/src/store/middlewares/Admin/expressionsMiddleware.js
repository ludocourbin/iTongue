...
const expressionsMiddleware = (store) => (next) => (action) => {
    ...
    switch (action.type) {
        ...
        case SET_TRADUCTIONS_BY_EXPRESSION : {
            ...
            const findTraductionsByExpression = expressionsList.find(expression => {
            ...
            store.dispatch(setTraductionsByExpressionSuccess(findTraductionsByExpression.traductions));
            break;
        };
        case ADD_TRADUCTION_SUBMIT: {
            
            const { expressionsList, expressionId, newTraductionInputValue } = store.getState().expressionsReducer;
            ...
            const expressionListWithNewTrad = expressionsList.map((expression) => {
                if( expression.id === expressionId ) {
                    return {
                        ...expression,
                        traductions: [
                            ...expression.traductions,
                            {
                                ...
                                traduction: newTraductionInputValue.traduction,
                            },
                        ],
                    };
                }
        ...
        case DELETE_TRADUCTION : {
            ...
            const removeTraduction = findExpression.traductions.filter(traduction => {
        ...
        case ADD_EXPRESSION_SUBMIT: {
            ...
            const tempCreateFakeId = () => {
                return store.getState().expressionsReducer.expressionsList.length + 1;
            };
