                ...
                if (contact && (authorId == contact.id || authorId == currentUsr.id)) {
                  ...
                  if (allThreads) {
                    ...
                    const threadIndex = newThreads.findIndex(
                      thread => thread.contact.id == authorId
                    );
                    ...
                    if (threadIndex > -1) {
                      ...
                      newThreads.push({
                        ...
                        messages: [{ text, sender: { id: authorId } }],
                        latest: new Date()
                      });
                    }
            ...
            if (count < 99)
              return (
                ...
                thread.messages.filter(
                  message => message.recipient.id == currentUser.id && !message.read
                ).length
              );
          }, 0);
...
function getAccessToken(store) {
  return new Promise((resolve, reject) => {
    ...
    if (!refreshToken)
      return reject(new Error("Le token ne peut être renouvelé en l'absence de refreshToken"));
